...
class Document(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.FirstSelBlock = None  # 当前获得焦点的块，也是第一个选中的块
        self.SelBlocks = []  # 所有选中的块

        # 新建页面
        self.RootPage = self.LastPage = None
        page = self.addPage(None)  # 新建第一页
    ...
    def addTextBlockWithTextItem(self, block=False, text=""):  # block为False表示在当前段落后添加abc_我的世界1
        ...
        # 测试
        # GlobalVars.CurrentFont = QFont("微软雅黑", pointSize=12, weight=QFont.Bold, italic=True)
        # GlobalVars.CurrentBackgroundColor = QColor(255, 0, 255)
        # newTextBlock.addTextItem(text)
        # GlobalVars.CurrentTextColor = QColor(255, 255, 0)
        # newTextBlock.addTextItem(text)
        # GlobalVars.CurrentTextColor = QColor(0, 0, 255)
        # GlobalVars.CurrentBackgroundColor = QColor(0, 255, 255)
        # GlobalVars.CurrentFont = QFont("宋体", pointSize=15)
        # newTextBlock.addTextItem(text)
        # GlobalVars.CurrentTextColor = QColor(0, 0, 255)
        # GlobalVars.CurrentFont = QFont("宋体", pointSize=20)
        # newTextBlock.addTextItem(text)
        return newTextBlock
        ...
        if y1 > 3 or x1 > 2:  # 向下选择
            ...
            while block:
                if block.posY[1] > y:
                    ...
                    break
            self.SelBlocks[0].SelStatus = SelStatus.SelPart  # 有缺陷，认为不肯能恰好完整的选取选中的第一段和最后一段
            self.SelBlocks[len(self.SelBlocks) - 1].SelStatus = SelStatus.SelPart
            for b in self.SelBlocks:  # 可优化，与上面合并，但会另增判断
                b.upSelEvent()  # 块向下选中时的事件
